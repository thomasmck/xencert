...
class StorageHandler:
    ...
    def MPConfigVerificationTests(self):
        try:
            ...
            if self.storage_conf['storage_type'] in ['lvmohba', 'lvmofcoe'] and self.storage_conf['pathInfo'] == None:
                raise Exception("Path related information not specified for storage type lvmohba/lvmofcoe.")
...
class StorageHandlerHBA(StorageHandler):
    def __init__(self, storage_conf):
        ...
        StorageHandler.__init__(self, storage_conf)
        self.sr_type = "lvmohba"

    def Create_SR(self):
        return self.session.xenapi.SR.create(util.get_localhost_uuid(self.session), \
            self.device_config, '0', 'XenCertTestSR', 'XenCertTestSR-desc', self.sr_type, '',False, {})
    ...
    def Create(self):
        ...
        try:
            ...
            (retVal, listAdapters, listSCSIId) = StorageHandlerUtil. \
                                               ...
                                               self.storage_conf, self.sr_type)
            if not retVal:                
...
class StorageHandlerFCOE(StorageHandlerHBA):
    def __init__(self, storage_conf):
        XenCertPrint("Reached StorageHandlerHBA constructor")
        StorageHandler.__init__(self, storage_conf)
        self.sr_type = "lvmofcoe"
